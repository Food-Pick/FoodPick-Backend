...
import {
  ...
  GeminiApiResponse,
} from './gemini-recommend.service'; // 새로 추가할 Gemini 서비스
...
export class RecommendController {
  ...
  async getRecommendations(@Query() query: GetRecommendationDto): Promise<{
    ...
    this.logger.log(
      `Recommendation request received. Type: ${recommendationType}, Lat: ${lat}, Lon: ${lon}`,
    );
    ...
    if (recommendationType === 'gemini') {
      ...
      if (
        ...
        !userPricePreference ||
        !userFoodCategoryPreference ||
        userFoodCategoryPreference.length === 0
      ) {
        throw new BadRequestException(
          'Gemini 추천을 위해서는 userAgeGroup, userPricePreference, userFoodCategoryPreference(최소 1개)가 필수입니다.',
        );
      }
      ...
      return this.recommendService.getRecommendations(
        lat,
        lon,
        currentMealTime,
      );
